...
export default function ProfilePage() {
  ...
  const menuItems = [
    {
      ...
      href: '', // Not clickable - auto verified
    ...
    },
    {
      ...
      href: 'https://t.me/TetherlyOnline',
  ...
  return (
    <div className="min-h-screen pb-28 px-5 pt-4" style={{ background: '#f3f5f7' }}>
      ...
      {/* Admin panel removed from user profile - only accessible via /tetherly-master-control/ */}
        ...
        {menuItems.map((item, i) => (
          <button
            ...
            style={{
              ...
              opacity: item.href ? 1 : 0.85,
            }}
            disabled={!item.href}
          >
            <div className="w-10 text-center shrink-0" style={{ color: '#666' }}>{item.icon}</div>
            <span className="flex-1 text-left text-[15px] font-semibold" style={{ color: '#333' }}>{item.label}</span>
            {item.status && (
      ...
      >
        ...
        <span className="flex-1 text-left text-[15px] font-semibold" style={{ color: '#ff4d4d' }}>Logout</span>
      </button>
